...
> **BETA in 00-social-content** — This skill is shipped as a beta inside
> 00-social-content. The Playwright-based renderer requires a working Node.js
> install and may need additional setup on first run. Validate output before
> relying on it for production carousels.
...
`.excalidraw` JSON files saved to `{projects_base}/viz-excalidraw-diagram/{YYYY-MM-DD}/{diagram-name}/`, rendered to PNG for validation.
...
Save to `{projects_base}/viz-excalidraw-diagram/{YYYY-MM-DD}/{diagram-name}/`:
- `{descriptive-name}.excalidraw` — the source JSON
- `{descriptive-name}.png` — the rendered output
