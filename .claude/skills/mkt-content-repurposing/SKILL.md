...
description: >
  ...
  Saves per-platform files to {projects_base}/mkt-content-repurposing/.
...
One piece of source content turned into 10-16 platform-native assets, saved to `{projects_base}/mkt-content-repurposing/{YYYY-MM-DD}/{campaign-name}/`. Each file is ready to copy-paste and publish — not a reworded version of the same post, but content that feels like it belongs on each platform.
...
Look for `{brand_context}/voice-profile.md` at the project root (the working directory where Claude Code was launched). If the file is missing, the skill proceeds with neutral defaults — never blocks. If a downstream user wants brand-matched output, suggest running `/mkt-brand-voice` once before the next invocation.
...
If they give a URL, try WebFetch first. If that fails (JS-heavy site, bot protection), fall back to `firecrawl-scraper` (needs `FIRECRAWL_API_KEY` in `{env_file}`). If both fail, ask them to paste the content.
...
Create: `{projects_base}/mkt-content-repurposing/{YYYY-MM-DD}/{campaign-name}/`
