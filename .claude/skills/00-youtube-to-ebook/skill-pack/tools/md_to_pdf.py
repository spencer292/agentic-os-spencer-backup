...
Usage:
    ...
    python3 md_to_pdf.py input.md output.pdf --theme branded --tokens visual-identity/tokens.json
    python3 md_to_pdf.py input.md output.pdf --theme branded --tokens visual-identity/tokens.json \
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tokens",
        help="Path to brand tokens.json (or legacy design-tokens.md), required when --theme branded",
        default=None,
    )
    parser.add_argument(
        "--intensity",
    ...
    if args.theme == "branded" and not args.tokens:
        print("ERROR: --theme branded requires --tokens <path-to-tokens.json>")
        sys.exit(1)
