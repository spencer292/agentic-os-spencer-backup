...
FACE_ZOOM_MULT = 2.5
FACE_CROP_MIN_W = 250
FACE_CROP_MAX_W_RATIO = 0.35
FACE_CROP_MAX_H_RATIO = 0.50

# Scene classification: position-primary, size-secondary.
# Face on the side = screen-share (content visible). Face centered = talking-head.
CX_SIDE_THRESHOLD = 0.30   # cx < this or cx > (1-this) = face is to one side
CX_CENTER_LOW = 0.35       # cx between these = centered (talking-head)
CX_CENTER_HIGH = 0.65
FW_THRESHOLD = 0.08        # only used as tiebreaker: tiny face + centered = screen-share


def _detect_faces_all(video_path, info, net):
    """Detect faces on every frame. Returns (cx, cy, fw, conf arrays)."""
    ...
    cx_list, cy_list, fw_list, conf_list = [], [], [], []
    idx = 0
    ...
    return raw_cx, raw_cy, raw_fw, raw_conf


GRAPHIC_CONF_THRESHOLD = 0.3
GRAPHIC_MIN_FRAMES = 5


def _detect_graphic_inserts(conf_array, fps):
    """Find frame ranges where face disappears, indicating a full-screen graphic."""
    inserts = []
    in_graphic = False
    start = 0

    for i, c in enumerate(conf_array):
        if c < GRAPHIC_CONF_THRESHOLD:
            if not in_graphic:
                in_graphic = True
                start = i
        else:
            if in_graphic:
                duration = i - start
                if duration >= GRAPHIC_MIN_FRAMES:
                    inserts.append((start, i - 1))
                in_graphic = False

    if in_graphic:
        duration = len(conf_array) - start
        if duration >= GRAPHIC_MIN_FRAMES:
            inserts.append((start, len(conf_array) - 1))
...
def _render_adaptive(video_path, output_path, face_cx_smooth, face_cy_smooth,
                     ...
                     scenes, src_w, src_h, fps, total, graphic_inserts=None):
    """Render: split-screen for screen-share, single face crop for talking-head."""
    frame_bytes = src_w * src_h * 3

    graphic_frames = set()
    if graphic_inserts:
        for gs, ge in graphic_inserts:
            for f in range(gs, ge + 1):
                graphic_frames.add(f)

    SCREEN_ZOOM_OUT = 1.10
    screen_crop_h = min(src_h, int(src_h * SCREEN_ZOOM_OUT))
    screen_crop_w = min(src_w, int(screen_crop_h * SCREEN_ASPECT))
    ...
    # Pre-compute letterbox dimensions for full-frame fallback (graphic inserts)
    letterbox_scale = min(OUT_W / src_w, OUT_H / src_h)
    letterbox_w = int(src_w * letterbox_scale)
    letterbox_h = int(src_h * letterbox_scale)
    letterbox_x = (OUT_W - letterbox_w) // 2
    letterbox_y = (OUT_H - letterbox_h) // 2
    ...
    while True:
        ...
        if mode == 1 and i in graphic_frames:
            # GRAPHIC INSERT: show full source frame, letterboxed to 9:16
            output_frame[:] = 0
            scaled = cv2.resize(frame, (letterbox_w, letterbox_h),
                                interpolation=cv2.INTER_LANCZOS4)
            output_frame[letterbox_y:letterbox_y + letterbox_h,
                         letterbox_x:letterbox_x + letterbox_w] = scaled
        elif mode == 1:
            ...
            fcw = min(fcw, int(src_w * FACE_CROP_MAX_W_RATIO))
            fch = min(fch, int(src_h * FACE_CROP_MAX_H_RATIO))
            fcw = min(fcw, src_w)
            fch = min(fch, src_h)

            fx = int(cx * src_w - fcw / 2)
            fy = int(cy * src_h - fch * 0.40)
...
def reframe(video_path, output_path):
    ...
    raw_cx, raw_cy, raw_fw, raw_conf = _detect_faces_all(video_path, info, net)
    total = len(raw_cx)

    graphic_inserts = _detect_graphic_inserts(raw_conf, fps)
    if graphic_inserts:
        total_graphic_frames = sum(ge - gs + 1 for gs, ge in graphic_inserts)
        print(f"  [split-screen] Detected {len(graphic_inserts)} graphic inserts "
              f"({total_graphic_frames} frames)")
    ...
    for s_start, s_end in scenes:
        ...
        # Position-primary classifier: face to the side = screen-share
        if avg_cx < CX_SIDE_THRESHOLD or avg_cx > (1 - CX_SIDE_THRESHOLD):
            raw_mode = 0  # screen-share — face is off to one side
        elif CX_CENTER_LOW <= avg_cx <= CX_CENTER_HIGH:
            raw_mode = 1  # talking-head — face is centered
        else:
            # Ambiguous zone (0.30-0.35 or 0.65-0.70) — use face width as tiebreaker
            raw_mode = 0 if avg_fw > FW_THRESHOLD else 1
    ...
    for idx, (s_start, s_end) in enumerate(scenes):
        ...
        if mode_val == 1:
            ...
            median_fw = np.nanmedian(seg_fw)
            crop_w = max(FACE_CROP_MIN_W, int(median_fw * w * FACE_ZOOM_MULT))
            crop_w = min(crop_w, w)
    ...
    _render_adaptive(video_path, output_path, face_cx_smooth, face_cy_smooth,
                     mode_per_frame, scene_face_crop_w, screen_x_per_frame,
                     scenes, w, h, fps, total, graphic_inserts=graphic_inserts)
