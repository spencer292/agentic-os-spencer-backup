...
- TEXT robot LIVE (jobber-text-automation), browser-only. 8/11 = 40 texts/$5,914/0 fail. Open: 28 rolled ($2,475), 8 comml ($3,580).
...
- Wk 08-17 SHIPPED: 565 visits timed, 34 windows, 4 dups gone. OT 8.8→4.2h; Alias 3.4h unfixable (T1=85.9h/2). Cluster pricing live (Barbee Mill 120min/11) — sweep for more.
- ALL CRONS STOPPED at the runtime — restart `scripts/start-crons.sh`. Never re-enable in reconciliation: route-drift-check, jobber-visit-followups, route-horizon-extend.
- Mo vs Spencer close = artifact; real gap is Spencer BOOKS on the call (64% vs 17%), 3.3 vs 1.7 touches. quote-chase cron LIVE 09:00 wkdys → both get quotes 24h+ unopened.
