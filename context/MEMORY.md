...
- **DEFECT (live, books nightly): cadence engine ignores MISSES.** decide.mjs `active = caught || ACTIVE_CODES.has(activity)` skips parsed misses → TMCP `Missed 3`+`NA` = MONTHLY, opposite of the 08-06 rule.
...
- TMCP 8/5: MRR $73.1K, 661 jobs/642 clients; tag=per-CLIENT NEVER=jobs. QF on-close=OUTSIDE MRR. Open: Kaiser#5390 dbl-tag; "FINAL BILL" phantoms.
- NS1: $857K T12 +37%; close 69%, gap=quote-ISSUE. NOW: LSA(gated)+pricing A-F. NS2: kits LIVE Gumroad, $0 rev; Ads+GSC OAuth expired 4wk.
...
- n8n: Jobber cred dead 7/24. Lead Alert blocked on LEAD_ALERT_SMTP_* pw. Jobber dupes: 97grp/114 extra — briefs/jobber-duplicate-cleanup.
