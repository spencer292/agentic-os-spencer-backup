...
// 2026-09-17: this passed a hard-coded balancing:'OFF' while --balancing (default ON) was used only
// in the three console.log lines above, so the script announced "balancing=ON" and then planned with
// it off. That defeats the one thing this script exists for — the week-of-09-21 run it produced
// Alias 11.7h mon / 3.6h tue / 12.1h wed / 3.1h thu and reported "9 tech-day(s) over the 9h ceiling"
// while writing it back anyway. Balancing is safe here precisely because assignedTo is set on every
// order above: the optimizer may move a stop between that tech's DAYS but can never hand the
// customer to anyone else. NOTE --max-day-hours is still report-only (see line ~519) — it counts
// breaches, it does not constrain the planner.
const sp = await orCall('start_planning', { dateRange: { from: FROM, to: TO }, balancing: BALANCING, clustering: true, startWith: 'CURRENT', lockType: 'NONE' });
