...
import { normAddr, loadCache, saveCache, classifyPoint, crossesOwners } from './geo-side.mjs';
...
for (const [ln, list] of Object.entries(byLine)) {
  ...
  // Derived, not stored: ownership moves on handover dates, so whether a line changes the OWNER
  // depends on WHEN. The stored crossesOwners flag was deleted 2026-08-21 (spec v2 D3) after it was
  // found disagreeing with the handover table it was supposed to summarise.
  const xo = crossesOwners(T, ln, process.argv.find(a => a.startsWith('--to='))?.slice(5));
  if (xo.crosses) {
    console.log('   (this line changes the OWNER, not just the day: ' + Object.entries(xo.sides).map(([k,v])=>k+'='+v).join(', ') + ')');
