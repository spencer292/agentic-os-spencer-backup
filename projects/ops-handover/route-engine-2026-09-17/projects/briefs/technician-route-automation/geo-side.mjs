...
// ---------------------------------------------------------------------------
// Does a geoSplit line change the OWNER, or only the weekday?
//
// This used to be STORED on the line as `crossesOwners`, and storing it is what made it wrong:
// ownership moves on handover dates, so the answer is a function of the date and cannot be a
// constant. On 2026-08-21 the stored flags had drifted from the handover table in two places —
// `ownerFrom20260817` claimed Tavis Alexander owned "North of SR-516" and "Issaquah South" while
// the handovers (the thing the code actually reads) gave both to Robert Norton. Route-engine spec
// v2 defect D3 read the stale flag and concluded sr-516 was mismarked; it was not. Deriving it
// removes the class of bug rather than correcting one instance of it.
//
// Returns { crosses, sides: { SIDE: owner } } for the given line at the given date.
export function crossesOwners(T, lineName, atDate) {
  const date = String(atDate || new Date().toISOString().slice(0, 10)).slice(0, 10);
  const ownerAt = (regionName) => {
    const r = T.regions?.[regionName];
    if (!r) return null;
    let owner = r.owner;
    for (const h of T.handovers || []) {
      if (h.regions?.includes(regionName) && date >= h.effective) owner = h.to;
    }
    return owner;
  };
  const sides = {};
  for (const [name, r] of Object.entries(T.regions || {})) {
    if (r.geoSplit?.line !== lineName) continue;
    sides[r.geoSplit.side] = ownerAt(name);
  }
  const owners = [...new Set(Object.values(sides).filter(Boolean))];
  return { crosses: owners.length > 1, sides };
}
