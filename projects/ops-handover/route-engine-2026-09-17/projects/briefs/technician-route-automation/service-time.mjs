...
// tech|date -> { check, set, reason }. A ride-along training day is planned at the pace being
// trained, not the pace either tech runs alone.
const DAY_OVERRIDE = {};
for (const o of CONFIG.dayOverrides || []) {
  DAY_OVERRIDE[`${(o.tech || '').trim().toLowerCase()}|${o.date}`] = o;
}

/** The dated pace override for this tech on this date, or null. */
export function dayOverrideFor(tech, date) {
  if (!tech || !date) return null;
  return DAY_OVERRIDE[`${tech.trim().toLowerCase()}|${date}`] || null;
}
 ...
 * Precedence: cluster > dayOverride (tech+date) > tech > default.
 */
export function serviceDuration(tech, isSet, jobNumber, visitDate) {
  const cluster = clusterFor(jobNumber);
  if (cluster) return cluster.minutes;
  const ov = dayOverrideFor(tech, visitDate);
  if (ov) return isSet ? ov.set : ov.check;
...
export function serviceTimeSummary() {
  ...
  for (const o of CONFIG.dayOverrides || []) {
    out += `
day override: ${o.tech} on ${o.date} planned at ${o.check}/${o.set} min — ${o.reason || 'no reason recorded'}`;
  }
  for (const [name, c] of Object.entries(CONFIG.clusters || {})) {
