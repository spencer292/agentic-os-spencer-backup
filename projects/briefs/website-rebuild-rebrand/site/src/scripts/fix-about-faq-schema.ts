/**
 * fix-about-faq-schema.ts
 *
 * One-shot fix: on the /about/ CMS page, walk the layout and set every
 * FAQ block's generateSchema to false. The page route already aggregates
 ...
 *
 * USAGE
 *   npm run fix-about-faq                     (dry-run, default)
 *   npm run fix-about-faq -- --apply          (write)
 */
...
async function run() {
  ...
  if (!doc) {
    console.error('Could not find pages/about published record')
    process.exit(1)
  }

  const layout = doc.layout as Array<Record<string, unknown>> | undefined
  if (!Array.isArray(layout)) {
    console.error('pages/about has no layout array')
    process.exit(1)
  }
  ...
  console.log(`Done. Updated pages/about (id=${doc.id}).`)
  process.exit(0)
}
