...
- [x] **OAuth creds + `.env` — DONE 2026-07-22 night:** Spencer created the Desktop OAuth client + added client ID/secret/dev token; Claude minted the refresh token via `scripts/rr-mint-refresh-token.mjs` (written straight to `.env`, never displayed), sent + API-accepted the MCC link (763-085-7815 now ACTIVE under 143-307-0544), and `scripts/rr-ads-smoke-test.mjs` passes: account reachable, 0 campaigns. **API access is live.** Next: build launch Search campaign (paused, $2.47/day) for sign-off, then flip `ztb-ads-manager` cron active. Original steps kept below for reference:
